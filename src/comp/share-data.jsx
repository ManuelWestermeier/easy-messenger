    ...
    const send = async (e) => {
        ...
        for (const chatId of selectedChats) {
            ...
            setChats((oldData) => ({
                ...
                [chatId]: {
                    ...oldData[chatId],
                    messages: [...(oldData[chatId]?.messages || []), { ...message, ...messagePublic }],
                },
            }));

            const encryptedMessage = encrypt(chatData.password, JSON.stringify(message));

            const isSent = await client.get("send", { chatId, message: encryptedMessage, ...messagePublic });
            if (!isSent) {
                alert(`A send error occurred for chat ${chatData.chatName}. Your message wasn't sent!`);
            }
        }
    ...
    return (
        <div className="share-container">
            <div className="share-header">
                <h1>Share...</h1>
                <button className="danger" onClick={() => setShareData(false)}>
